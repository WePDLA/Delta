    ...
    var AnnotatorUI = function(dispatcher, svg) {
      ...
      var fillSpanTypesAndDisplayForm = function(evt, spanText, span) {
        ...
        if (span) {
          ...
          spanForm.dialog('option', { title: '新标注' });
        }
        if (hideFrame == 'event') {
...
//                        true, true);
      ...
      var fillArcTypesAndDisplayForm = function(evt, originType, targetType, arcType, arcId) {
        ...
        if (arcId) {
          ...
          arcForm.dialog('option', { title: '编辑标注' });
        } else {
          // new arc
          ...
          arcForm.dialog('option', { title: '新标注' });
        }
        if (reversalPossible) {
      ...
      var deleteArc = function(evt) {
        if (Configuration.confirmModeOn && !confirm("你确定想删除该标注吗?")) {
          return;
        }
        var eventDataId = $(evt.target).attr('data-arc-ed');
        dispatcher.post('hideForm');
      ...
      $('#arc_form_delete').attr('title', '删除该标注.');
...
*/
      ...
      // 新建文件的操作
      $('#annotation_submit').click(function() {
        dispatcher.post('hideForm');
        dispatcher.post('showForm', [importForm]);
        importForm.find('input, textarea').val('');
      });
      ...
      // var submitForm = $('#complete_submit');
      // var submitFormSubmit = function(evt) {
      //   var _docid = "text_1";
      //   var _doctext = "text_content";
      //   var opts = {
      //     action : 'test',
      //     collection : coll,
      //     docid  : _docid,
      ...
      //   return false;
      // };
      // submitForm.submit(submitFormSubmit);
