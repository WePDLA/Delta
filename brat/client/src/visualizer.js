    ...
    var Visualizer = function(dispatcher, svgId, webFontURLs) {
      ...
      var setData = function(_sourceData) {
        ...
        $.each(sourceData.modifications, function(modNo, mod) {
          ...
          if (!data.spans[mod[2]]) {
            dispatcher.post('messages', [[['<strong>错误</strong><br/>Event ' + mod[2] + ' (referenced from modification ' + mod[0] + ') does not occur in document ' + data.document + '<br/>(please correct the source data)', 'error', 5]]]);
            return;
          }
        ...
        $.each(sourceData.attributes, function(attrNo, attr) {
          ...
          if (!span) {
            dispatcher.post('messages', [[['标注 ' + attr[2] + ', referenced from attribute ' + attr[0] + ', does not exist.', 'error']]]);
            return;
          }
